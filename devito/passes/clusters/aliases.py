...
from devito.tools import as_list, flatten, split
...
def optimize_schedule(cluster, schedule, platform, sregistry, options):
    ...
    if isinstance(rotate, str):
        for r in rotate:
            def key(writeto):
                for n, i in enumerate(writeto):
                    if i.dim.name == r:
                        return n
            schedule = _optimize_schedule_rotations(schedule, key, sregistry)
    elif rotate:
        def key(writeto):
            return 0
        schedule = _optimize_schedule_rotations(schedule, key, sregistry)
...
def _optimize_schedule_rotations(schedule, key, sregistry):
    """
    ...
    for k, group in groupby(schedule, key=lambda i: i.writeto):
        ...
        # The rotation Dimension index
        ridx = key(k)

        # The rotation Dimension cannot be the innermost one
        if ridx is None or ridx + 1 >= len(k):
            processed.extend(g)
            continue

        # All preceding Dimensions must be fusible (or already fused through
        # a previous call to this same routine)
        if any(i != i.zero() for i in k[:ridx]):
            processed.extend(g)
            continue
